...
from gws_omix import FastaFile, GmapAlignFasta


class TestGmapFasta(BaseTestCase):

    def test_GmapFasta(self):
...
# "cross-species":"Yes", "alt-start-codons":"Yes", "fulllength":"Yes"
        ...
        tester = TaskRunner(
            ...
            task_type=GmapAlignFasta
        )

        outputs = tester.run()
        gff3_file = outputs['gmap_fasta_file']
        result_content = gff3_file.read()

        # Get the expected file output
        expected_file_path = os.path.join(data_dir, "test.gmap.fasta")
        ...
        self.assertEqual(text_view_dict["total_number_of_items"], 1319)
