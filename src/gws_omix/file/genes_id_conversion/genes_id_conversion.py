...
class IDConvertTask(Task):
    ...
    input_specs: Final[InputSpecs] = InputSpecs({
        "table_file": InputSpec((Table, File), human_name="Input table (CSV/TSV)", short_description="Must contain the column to convert."),
    })
    ...
    def run(self, p: ConfigParams, ins: TaskInputs) -> TaskOutputs:
        # Handle both Table and File inputs
        input_resource = ins["table_file"]
        if isinstance(input_resource, Table):
            # Convert Table to CSV File
            temp_file: File = TableExporter.call(input_resource, {"file_format": "csv"})
            infile = temp_file.path
        elif isinstance(input_resource, File):
            infile = input_resource.path
        else:
            raise RuntimeError("Unsupported type for 'table_file' (expected Table or File).")
