...
from gws_core import (File, FloatParam, IntParam, TaskInputs, TaskOutputs,
...
class ParsedOverlappingGmapHits(BaseOmixEnvTask):
    ...
    input_specs: InputSpecs == {
        'gmap_gff3_file': InputSpec(GFF3File,)
    }
    output_specs: OutputSpecs = {
        'Parsed_Gmap_gtf_file': OutputSpec(GTFFile,)
    }
    config_specs: ConfigSpecs = {
        "A_overlap":
        FloatParam(
            default_value=0.5, min_value=0.0, max_value=1.0,
            short_description="Proportion of feature A overlapped by feature B (default=0.5, 0.0 would means no overlap)."),
        "B_overlap":
        FloatParam(
            default_value=0.5, min_value=0.0, max_value=1.0,
            short_description="Proportion of feature A overlapped by feature B (default=0.5, 0.0 would means no overlap)."), }
