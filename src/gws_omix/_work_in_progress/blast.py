...
from gws_core import (ConfigParams, File, FloatParam, IntParam, StrParam,
...
class Blast(BaseOmixEnvTask):
    ...
    input_specs: InputSpecs = {
        'fasta_file': InputSpec(FastaFile,)
    }
    output_specs: OutputSpecs = {
        'filtered_blast_file': OutputSpec(File,)
    }
    ...
    def build_command(self, params: ConfigParams, inputs: TaskInputs) -> list:
        ...
        cmd = [
            blast_type, " -db ", datab_file_path, " -query ", fasta_file.path, " -evalue ", evalue, " -num_threads ",
            thread, " -qcov_hsp_perc ", cov, " -num_alignments ", num_alignments,
            "-outfmt  \"7 qaccver saccver pident qcovs qcovhsp length mismatch gapopen qstart qend qlen sstart send slen evalue bitscore\" -show_gis -task \"blastp-fast\" -out ",
