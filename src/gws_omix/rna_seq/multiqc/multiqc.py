...
class MultiQc(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        fastqc_files = [f for f in os.listdir(input_folder.path) if qc_pattern.match(f)]

        if sequencing_mode == "single":
            # Vérifier qu'on a bien trouvé des fichiers
            if not fastqc_files:
                raise Exception("Aucun rapport FastQC trouvé pour les fichiers single-end.")
            ...
            if not r1_files or not r2_files:
                raise Exception("Aucun rapport FastQC trouvé pour les fichiers paired-end.")
            ...
            if res_forward != 0 or res_reverse != 0 or res_combined != 0:
                raise Exception("Une erreur est survenue lors de la génération des rapports MultiQC.")
