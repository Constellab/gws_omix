...
do
    htseq-count -n $thrds  --stranded=no -m intersection-nonempty $i $annotation | awk -v sample=$(basename $i) 'BEGIN{print "Name\t"sample}{print $0;}'  > $(basename $i).htseq-count.txt ; 
