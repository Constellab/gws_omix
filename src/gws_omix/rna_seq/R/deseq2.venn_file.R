...
if (length(args)==0) { # test if there is at least one argument: if not, return an error
  ...
  Design = paste(" ~ ",args[3]) # " + " # see: https://cran.r-project.org/doc/manuals/R-intro.html#Formulae-for-statistical-models
  ...
  for (j in 1:length(uniq)){
    if( i != j ){ 
      if( j %in% mylist == TRUE){
        ...
        res <- results(dds)       
        res <- subset(res$padj, res$padj < 0.05) # adjust p-value threshold to only keep differentaily expressed genes in a given condition
        if(old_res):
            old_res <- merge(x = old_res, y = res, by = "row.names", all = TRUE)
...
write.table(old_res, file = paste(output_file,"pvaladj_merged",metadata_col,"complete","txt", sep='.'), quote=F, sep="\t")
