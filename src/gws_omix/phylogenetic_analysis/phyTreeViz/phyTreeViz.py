...
@task_decorator(
    "PhyTreeViz",
    human_name="Phylogenetic Tree Visualization",
...
class PhyTreeViz(Task):
    ...
    **Behaviour**
    ...
    - Branch lengths from the Newick file are **always used** to define the
      horizontal scale of the tree (no option to ignore them).
    - You can optionally display the numeric branch lengths on edges via
      `show_branch_length`, but this does *not* affect the geometry.
    ...
    **Main parameter**
    ...
    - `prefix` (string, default: tree filename stem)
      Base name for outputs. The final figure will be `<prefix>.tree.png`.
    ...
    **Advanced figure appearance parameters**
    ...
    - `show_branch_length`
      Show numeric branch lengths on edges (can be cluttered on large trees).
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if rc:
            raise RuntimeError("phyTreeViz worker failed (non-zero return code). "
                               "Check task logs for details.")

        if not fig_path.is_file():
            # For debugging, list files in the working directory
            try:
